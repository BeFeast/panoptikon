...
function HeroStatSkeleton() {
  return (
    <Card className="border-0 bg-slate-800/50 h-full min-h-[7.5rem]">
...
const QUICK_ACTIONS = [
  { label: "Scan Network", href: "/devices", icon: Radar },
  { label: "View Alerts", href: "/alerts", icon: AlertTriangle },
  { label: "Check DNS", href: "/dns", icon: Search },
] as const;
...
export default function DashboardPage() {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {/* ── Hero Stats Row ──────────────────────────────── */}
      <StaggerContainer className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" data-testid="hero-stats">
        {statsError ? (
          <>
            <StaggerItem>
              <HeroStat
          ...
          <>
            <StaggerItem>
              <HeroStat
                ...
                subtitle={`${stats.devices_online} online now`}
                icon={<MonitorSmartphone className="h-5 w-5" />}
                gradient="bg-gradient-to-br from-blue-600/80 to-blue-900/80"
                href="/devices"
              />
            </StaggerItem>
            <StaggerItem>
              <HeroStat
                ...
                icon={<AlertTriangle className="h-5 w-5" />}
                gradient={stats.alerts_unread > 0
                  ? "bg-gradient-to-br from-amber-600/80 to-amber-900/80"
            ...
            <StaggerItem>
              <HeroStat
                title="Uptime"
                value={stats.critical_total > 0 ? Math.round((stats.critical_online / stats.critical_total) * 100) : 0}
                formattedValue={stats.critical_total > 0 ? `${Math.round((stats.critical_online / stats.critical_total) * 100)}%` : "N/A"}
                subtitle={`${stats.critical_online}/${stats.critical_total} critical online`}
                icon={<Shield className="h-5 w-5" />}
            ...
            <StaggerItem>
              <HeroStat
                ...
                formattedValue={`↓ ${formatBps(stats.wan_rx_bps)}`}
      ...
      {/* ── Bento Grid ─────────────────────────────────── */}
      <StaggerContainer className="grid grid-cols-1 gap-6 xl:grid-cols-6" data-testid="bento-grid">
        {/* ── Network Health Ring (2 col, 1 row) ─────────── */}
        ...
        {/* ── Alert Feed (2 col) ──────────────────────────── */}
        <StaggerItem className="xl:col-span-2">
          <Card className="h-full border-slate-700/50 bg-slate-900/55">
            <CardHeader className="pb-4">
        ...
        {/* ── Device Type Breakdown (full width) ──────── */}
