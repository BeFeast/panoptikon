...
test.describe("VPN Status Page — OpenVPN tab (#664)", () => {
  ...
  test("shows OpenVPN tab when OpenVPN server is active", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("2h15m")).toBeVisible();
  ...
  test("hides OpenVPN tab when no OpenVPN server is active", async ({
    page,
  }) => {
    await mockVpnStatus(page, MOCK_VPN_WITH_MIKROTIK_WG);
    await page.goto("/vpn-status/");
  ...
  test("displays per-tunnel bandwidth metrics for WireGuard peers", async ({
    page,
  }) => {
    await mockVpnStatus(page, MOCK_VPN_WITH_MIKROTIK_WG);
    await page.goto("/vpn-status/");
    ...
    // Check summary cards show bandwidth
  ...
  test("displays WireGuard peer handshake times", async ({ page }) => {
    ...
    // The peer should show a recent handshake time (within seconds)
    // The exact text depends on timing, but it should contain "ago"
    await expect(page.getByText(/\d+s ago/)).toBeVisible({ timeout: 5000 });
