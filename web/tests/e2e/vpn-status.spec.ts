 ...
 * and that the OpenVPN tab is shown when OpenVPN is enabled (#664).
 */
...
/** VPN status response with OpenVPN server and connected clients. */
...
test.describe("VPN Status Page — OpenVPN tab (#664)", () => {
  ...
  test("shows OpenVPN tab when OpenVPN server is available", async ({
    page,
  }) => {
  ...
  test("overview tab shows both WireGuard and OpenVPN when both available", async ({
    page,
  }) => {
    await mockVpnStatus(page, MOCK_VPN_WITH_OPENVPN);
    ...
    // Overview tab should show both interface cards
