...
test.describe("VPN Status Page — OpenVPN tab (#664)", () => {
  ...
  test("shows OpenVPN overview summary when enabled", async ({ page }) => {
    ...
    // Check for the OpenVPN connected clients summary in the overview
    await expect(
      page.getByRole("tabpanel").getByText("OpenVPN", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("tabpanel").getByText(/\d+ connected client/),
    ).toBeVisible();
