...
test.describe('Layout & Grid — card clipping / spacing regressions (#544)', () => {
  ...
  test('stat cards have consistent padding — no extra bottom space', async ({ page }) => {
    ...
    const statLabels = ['Total Devices', 'Active Alerts', 'Uptime', 'Traffic'];
...
test.describe('Layout & Grid — no overflow or clipping (#524)', () => {
  ...
  test('hero stat cards use overflow-hidden to prevent overflow', async ({ page }) => {
    await page.goto('/dashboard');
    await expect(page.getByText('Total Devices')).toBeVisible({ timeout: 10000 });

    // Verify that hero stat value elements have tabular-nums for numeric alignment
    // and the card uses overflow-hidden to prevent text from breaking layout.
    const statValues = page.locator('.tabular-nums');
    const count = await statValues.count();
    expect(count).toBeGreaterThan(0);

    // Cards themselves should contain overflow
    const heroCards = page.locator('[data-testid="hero-stats"] .overflow-hidden');
