...
/**
 * Coverage for the `Continue with SSO` action and `OR` divider on `/login`.
 * The block must stay hidden unless the backend `/api/v1/auth/status` response
 * advertises `sso_enabled: true`. Refs #768.
 */
test.describe("Login SSO visibility (#768)", () => {
  test("default backend state hides OR divider and SSO action", async ({
    ...
    // Password login affordances remain intact.
