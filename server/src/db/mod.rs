...
const DNS_QUERY_LOG_MIGRATION: &str = include_str!("migrations/023_dns_query_log.sql");
...
const DYNAMIC_DNS_MIGRATION: &str = include_str!("migrations/025_dynamic_dns.sql");
...
pub(crate) async fn run_migrations(pool: &SqlitePool) -> Result<()> {
    ...
    // Migration 023: DNS query log table.
    ...
    if !applied_23 {
        sqlx::raw_sql(DNS_QUERY_LOG_MIGRATION).execute(pool).await?;

        sqlx::query("INSERT INTO _migrations (version) VALUES (23)")
            .execute(pool)
            .await?;

        info!("Applied migration 023_dns_query_log.sql");
    }

    // Purge DNS query logs older than 7 days on startup.
    let dns_deleted =
        sqlx::query("DELETE FROM dns_query_log WHERE queried_at < datetime('now', '-7 days')")
            .execute(pool)
            .await?
            .rows_affected();
    if dns_deleted > 0 {
        info!(dns_deleted, "Purged old DNS query logs on startup");
    }

    // Migration 024: DNS blocklist manager tables.
    let applied_24: bool = sqlx::query("SELECT 1 FROM _migrations WHERE version = 24")
        .fetch_optional(pool)
        .await?
        .is_some();
    ...
    if !applied_24 {
        ...
        sqlx::query("INSERT INTO _migrations (version) VALUES (24)")
            .execute(pool)
            .await?;
    ...
    if !applied_25 {
        sqlx::raw_sql(DYNAMIC_DNS_MIGRATION).execute(pool).await?;

        sqlx::query("INSERT INTO _migrations (version) VALUES (25)")
            .execute(pool)
            .await?;

        info!("Applied migration 025_dynamic_dns.sql");
    }
