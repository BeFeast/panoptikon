...
use super::{audit, AppError, AppState};
...
#[derive(Debug, Deserialize)]
pub struct MikrotikUpdateDhcpServerRequest {
    pub lease_time: Option<String>,
    pub address_pool: Option<String>,
...
#[derive(Debug, Deserialize)]
pub struct MikrotikDhcpNetworkRequest {
    pub address: String,
    pub gateway: Option<String>,
    pub dns_server: Option<String>,
    pub domain: Option<String>,
...
#[derive(Debug, Deserialize)]
pub struct MikrotikIpPoolRequest {
    pub name: String,
    pub ranges: String,
    pub comment: Option<String>,
}
...
#[derive(Debug, Serialize, Deserialize)]
pub struct MikrotikDhcpLogEntry {
    pub id: Option<String>,
...
fn validate_vlan_upsert(body: &MikrotikVlanUpsertRequest) -> Result<(), AppError> {
    if !(1..=4094).contains(&body.vlan_id) {
        return Err(AppError::Validation(
            "VLAN ID must be between 1 and 4094".into(),
        ));
    }
    if body.name.trim().is_empty() || body.interface.trim().is_empty() {
        return Err(AppError::Validation(
            "VLAN name and interface are required".into(),
        ));
    }
    if matches!(body.mtu, Some(0)) {
        return Err(AppError::Validation("MTU must be greater than 0".into()));
    }
    Ok(())
}
...
pub async fn update_vlan(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("VLAN ID parameter is required".into()));
    }
...
pub async fn update_firewall_filter(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("Filter rule ID is required".into()));
    }
...
pub async fn update_firewall_nat(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("NAT rule ID is required".into()));
    }
...
pub async fn update_address_list(
    ...
    if id.is_empty() {
        return Err(AppError::Validation(
            "Address list entry ID is required".into(),
        ));
    }
...
pub async fn delete_dhcp_lease(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("DHCP lease ID is required".into()));
    }
...
pub async fn update_dhcp_server(
    Path(id): Path<String>,
    State(state): State<AppState>,
    Json(body): Json<MikrotikUpdateDhcpServerRequest>,
    ...
    let req = DhcpServerWriteRequest {
        ...
        disabled: body
            .disabled
            .map(|d| if d { "true" } else { "false" }.to_string()),
        authoritative: body.authoritative.clone(),
    };
...
pub async fn create_dhcp_network(
    State(state): State<AppState>,
    Json(body): Json<MikrotikDhcpNetworkRequest>,
) -> Result<StatusCode, StatusCode> {
    let client = mikrotik_client(&state)
        .await
        .ok_or(StatusCode::SERVICE_UNAVAILABLE)?;
...
pub async fn create_dhcp_pool(
    State(state): State<AppState>,
    Json(body): Json<MikrotikIpPoolRequest>,
) -> Result<StatusCode, StatusCode> {
    let client = mikrotik_client(&state)
        .await
        .ok_or(StatusCode::SERVICE_UNAVAILABLE)?;
...
/// PUT /api/v1/mikrotik/dhcp/pools/:id
...
/// GET /api/v1/mikrotik/dhcp/logs
pub async fn dhcp_logs(
    State(state): State<AppState>,
) -> Result<Json<Vec<MikrotikDhcpLogEntry>>, StatusCode> {
    let client = mikrotik_client(&state)
        .await
        .ok_or(StatusCode::SERVICE_UNAVAILABLE)?;

    let logs = client.dhcp_logs().await.map_err(|e| {
        tracing::error!("MikroTik DHCP logs error: {e}");
        StatusCode::BAD_GATEWAY
    })?;

    let result: Vec<MikrotikDhcpLogEntry> = logs
        .into_iter()
        .map(|l| MikrotikDhcpLogEntry {
            id: l.id,
            time: l.time,
            topics: l.topics,
            message: l.message,
...
pub async fn delete_mangle(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("Mangle rule ID is required".into()));
    }
...
pub async fn delete_routing_rule(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("Routing rule ID is required".into()));
    }
...
pub async fn delete_netwatch(
    ...
    let id = id.trim();
    if id.is_empty() {
        return Err(AppError::Validation("Netwatch entry ID is required".into()));
    }
