...
pub async fn get_settings(
    ...
    // Legacy Routers toggle.
    // If the setting has never been persisted, auto-enable when VyOS is already
    // configured so existing users keep their VyOS workflows without a manual opt-in.
    let show_legacy_routers = match get_setting(&state, "show_legacy_routers").await {
        Some(v) => v == "1" || v == "true",
        None => vyos_url.is_some() && vyos_api_key_set,
    };
