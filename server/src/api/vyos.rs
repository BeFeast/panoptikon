...
/// DNS forwarding configuration returned by the GET endpoint.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DnsConfig {
    pub nameservers: Vec<String>,
    pub domain_forwarding: Vec<DnsDomainForward>,
...
/// A per-domain forwarding rule (conditional DNS).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DnsDomainForward {
    pub domain: String,
    pub server: String,
}

/// A static hostname → IP override.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Parse VyOS DNS forwarding config JSON into our typed struct.
fn parse_dns_config(fwd: &Value, host_mappings: &Value) -> DnsConfig {
    ...
    if let Some(ns) = fwd.get("name-server") {
        match ns {
            Value::String(s) => {
                nameservers.push(s.clone());
            }
            Value::Object(map) => {
                for key in map.keys() {
                    nameservers.push(key.clone());
                }
            }
            Value::Array(arr) => {
                for v in arr {
    ...
    // Parse domain forwarding
    if let Some(Value::Object(domains)) = fwd.get("domain") {
        for (domain_name, domain_val) in domains {
            if let Some(Value::Object(servers)) = domain_val.get("server") {
                for server_ip in servers.keys() {
                    domain_forwarding.push(DnsDomainForward {
                        domain: domain_name.clone(),
                        server: server_ip.clone(),
                    });
                }
            } else if let Some(Value::String(server_ip)) = domain_val.get("server") {
                domain_forwarding.push(DnsDomainForward {
                    domain: domain_name.clone(),
                    server: server_ip.clone(),
                });
    ...
    if let Value::Object(hosts) = host_mappings {
        for (hostname, host_val) in hosts {
            if let Some(inet) = host_val.get("inet") {
                match inet {
                    Value::String(ip) => {
                        host_overrides.push(DnsHostOverride {
    ...
    host_overrides.sort_by(|a, b| a.hostname.cmp(&b.hostname));
...
pub async fn dns_config(State(state): State<AppState>) -> Result<Json<DnsConfig>, StatusCode> {
    ...
    let fwd = match client.retrieve(&["service", "dns", "forwarding"]).await {
        Ok(data) => data,
        Err(e) => {
            let msg = e.to_string();
            if msg.contains("empty") || msg.contains("does not exist") {
                ...
                tracing::error!("VyOS DNS forwarding query failed: {e}");
                return Err(StatusCode::BAD_GATEWAY);
            }
        }
    };

    // Fetch static host mappings
    let host_mappings = match client
        .retrieve(&["system", "static-host-mapping", "host-name"])
        .await
    {
        Ok(data) => data,
        Err(e) => {
            ...
            if msg.contains("empty") || msg.contains("does not exist") {
                ...
                tracing::error!("VyOS static host mapping query failed: {e}");
                return Err(StatusCode::BAD_GATEWAY);
            }
        }
    };

    Ok(Json(parse_dns_config(&fwd, &host_mappings)))
}

/// Request body for adding/removing a nameserver.
#[derive(Debug, Deserialize)]
pub struct DnsNameserverRequest {
    pub ip: String,
}

/// POST /api/v1/vyos/dns/nameservers — add an upstream nameserver.
pub async fn add_dns_nameserver(
    State(state): State<AppState>,
    Json(body): Json<DnsNameserverRequest>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
        (
            StatusCode::SERVICE_UNAVAILABLE,
    ...
    let description = format!("Add DNS nameserver {}", body.ip);
    let commands = vec![format!(
        "set service dns forwarding name-server {}",
        body.ip
    )];
    ...
    match client
        .configure_set(&["service", "dns", "forwarding", "name-server", &body.ip])
        .await
    {
        Ok(_) => {
            audit::log_success(&state.db, "dns_nameserver_add", &description, &commands).await;
            Ok(Json(VyosWriteResponse {
                success: true,
                message: format!("Nameserver {} added", body.ip),
            }))
        }
        Err(e) => {
            let msg = format!("VyOS error: {e}");
...
/// DELETE /api/v1/vyos/dns/nameservers/:ip — remove an upstream nameserver.
pub async fn delete_dns_nameserver(
    State(state): State<AppState>,
    Path(ip): Path<String>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
    ...
    let description = format!("Delete DNS nameserver {ip}");
    let commands = vec![format!("delete service dns forwarding name-server {ip}")];

    match client
        .configure_delete(&["service", "dns", "forwarding", "name-server", &ip])
        .await
    {
        Ok(_) => {
            audit::log_success(&state.db, "dns_nameserver_delete", &description, &commands).await;
            Ok(Json(VyosWriteResponse {
                success: true,
                message: format!("Nameserver {ip} deleted"),
            }))
        }
        Err(e) => {
            let msg = format!("VyOS error: {e}");
...
/// Request body for domain forwarding.
#[derive(Debug, Deserialize)]
pub struct DnsDomainForwardRequest {
    pub domain: String,
    pub server: String,
}
...
/// Validate a hostname (for host overrides, allows dots).
fn is_valid_hostname(hostname: &str) -> bool {
    is_valid_domain(hostname)
}

/// POST /api/v1/vyos/dns/domain-forwarding — add a domain forwarding rule.
pub async fn add_dns_domain_forward(
    State(state): State<AppState>,
    Json(body): Json<DnsDomainForwardRequest>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
        (
            StatusCode::SERVICE_UNAVAILABLE,
    ...
    if !is_valid_domain(&body.domain) {
        return Err((
            ...
            Json(VyosWriteResponse {
                success: false,
                message: format!("Invalid domain name: {}", body.domain),
            }),
        ));
    }
    ...
    if !is_valid_ip(&body.server) {
        return Err((
            ...
            Json(VyosWriteResponse {
                success: false,
                message: format!("Invalid server IP: {}", body.server),
            }),
        ));
    }

    let description = format!(
        "Add DNS domain forwarding: {} via {}",
        body.domain, body.server
    );
    let commands = vec![format!(
        "set service dns forwarding domain {} server {}",
...
/// DELETE /api/v1/vyos/dns/domain-forwarding/:domain — remove a domain forwarding rule.
pub async fn delete_dns_domain_forward(
    State(state): State<AppState>,
    Path(domain): Path<String>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
        ...
        .await
    {
        Ok(_) => {
            ...
            Ok(Json(VyosWriteResponse {
                success: true,
                message: format!("Domain forwarding for {domain} deleted"),
            }))
        }
        Err(e) => {
            let msg = format!("VyOS error: {e}");
...
/// Request body for a host override.
#[derive(Debug, Deserialize)]
pub struct DnsHostOverrideRequest {
    pub hostname: String,
    pub ip: String,
}

/// POST /api/v1/vyos/dns/host-overrides — add a local host override.
pub async fn add_dns_host_override(
    State(state): State<AppState>,
    Json(body): Json<DnsHostOverrideRequest>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
        (
            StatusCode::SERVICE_UNAVAILABLE,
    ...
    if !is_valid_hostname(&body.hostname) {
        return Err((
            ...
            Json(VyosWriteResponse {
                success: false,
                message: format!("Invalid hostname: {}", body.hostname),
            }),
        ));
    }
    ...
    let description = format!("Add DNS host override: {} → {}", body.hostname, body.ip);
    let commands = vec![format!(
...
/// DELETE /api/v1/vyos/dns/host-overrides/:hostname — remove a host override.
pub async fn delete_dns_host_override(
    State(state): State<AppState>,
    ...
    let description = format!("Delete DNS host override for {hostname}");
    let commands = vec![format!(
        ...
        .await
    {
        Ok(_) => {
            ...
            Ok(Json(VyosWriteResponse {
                success: true,
                message: format!("Host override for {hostname} deleted"),
            }))
        }
        Err(e) => {
            let msg = format!("VyOS error: {e}");
...
/// Request body for DNS settings update.
#[derive(Debug, Deserialize)]
...
/// PUT /api/v1/vyos/dns/settings — update DNS forwarding settings.
pub async fn update_dns_settings(
    State(state): State<AppState>,
    Json(body): Json<DnsSettingsRequest>,
) -> Result<Json<VyosWriteResponse>, (StatusCode, Json<VyosWriteResponse>)> {
    let client = get_vyos_client_or_503(&state).await.map_err(|_| {
        (
            StatusCode::SERVICE_UNAVAILABLE,
    ...
    if let Some(ref addr) = body.listen_address {
        if !addr.is_empty() && !is_valid_ip(addr) {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(VyosWriteResponse {
                    success: false,
                    message: format!("Invalid listen address: {addr}"),
                }),
            ));
        }
        ...
        if addr.is_empty() {
            ...
            audit_commands.push("delete service dns forwarding listen-address".to_string());
            if let Err(e) = client
                .configure_delete(&["service", "dns", "forwarding", "listen-address"])
                .await
            {
                let msg = format!("Failed to delete listen-address: {e}");
                audit::log_failure(
                    &state.db,
                    "dns_settings_update",
                    "Update DNS settings",
            ...
            if let Err(e) = client
                ...
                .await
            {
                let msg = format!("Failed to set listen-address: {e}");
                audit::log_failure(
                    &state.db,
                    "dns_settings_update",
                    "Update DNS settings",
    ...
    if let Some(size) = body.cache_size {
        ...
        if let Err(e) = client
            ...
            .await
        {
            let msg = format!("Failed to set cache-size: {e}");
            audit::log_failure(
                &state.db,
                "dns_settings_update",
                "Update DNS settings",
    ...
    // Update DNSSEC
    if let Some(ref dnssec) = body.dnssec {
        if !matches!(dnssec.as_str(), "off" | "process-no-validate" | "process") {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(VyosWriteResponse {
                    success: false,
                    message: format!("Invalid DNSSEC value: {dnssec}. Must be 'off', 'process-no-validate', or 'process'"),
                }),
            ));
        }
        audit_commands.push(format!("set service dns forwarding dnssec {dnssec}"));
        if let Err(e) = client
            .configure_set(&["service", "dns", "forwarding", "dnssec", dnssec])
            .await
        {
            let msg = format!("Failed to set dnssec: {e}");
            audit::log_failure(
                &state.db,
                "dns_settings_update",
                "Update DNS settings",
    ...
    audit::log_success(
        &state.db,
        "dns_settings_update",
        &format!("Update DNS settings ({changes} changes)"),
        &audit_commands,
    )
    .await;
    ...
    Ok(Json(VyosWriteResponse {
        success: true,
        message: format!("DNS settings updated ({changes} changes)"),
    }))
}
