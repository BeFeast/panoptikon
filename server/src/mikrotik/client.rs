...
impl MikrotikClient {
    ...
    /// Fetch routing rules (policy-based routing).
    ...
    /// Create an IP route.
    pub async fn create_ip_route(&self, req: &IpRouteWriteRequest) -> Result<()> {
        self.send_json(Method::POST, "/ip/route", req).await
    }

    /// Delete an IP route by RouterOS `.id`.
    pub async fn delete_ip_route(&self, id: &str) -> Result<()> {
        self.send_no_body(Method::DELETE, &format!("/ip/route/{id}"))
            .await
    }

    /// Fetch firewall mangle rules (for PBR marking).
    ...
    /// Fetch OSPF interface-templates.
    ...
    pub async fn ospf_neighbors(&self) -> Result<Vec<OspfNeighbor>> {
        let val = self.get("/routing/ospf/neighbor").await?;
        let res: Vec<OspfNeighbor> =
            serde_json::from_value(val).context("failed to parse OSPF neighbors")?;
        Ok(res)
    }
    ...
    /// Fetch BGP sessions.
    pub async fn bgp_sessions(&self) -> Result<Vec<BgpSession>> {
        let val = self.get("/routing/bgp/session").await?;
        let res: Vec<BgpSession> =
            serde_json::from_value(val).context("failed to parse BGP sessions")?;
        Ok(res)
    }
